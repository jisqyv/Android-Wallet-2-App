...
public class Transaction extends ChildMessage implements Serializable {
    ...
    public synchronized void signInputs(SigHash hashType, Wallet wallet) throws ScriptException {
        ...
        boolean[] compressed = new boolean[inputs.size()];
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            if (key == null) {
                key = wallet.findKeyFromPubHashCompressed(connectedPubKeyHash);
                compressed[i] = true;
            }
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            if (compressed[i])
                input.setScriptBytes(Script.createInputScript(signatures[i], key.getPubKeyCompressed()));
            else
